...
# 模型层包括pos_encoder（位置编码器）、transformer_encoder（变换器编码器）、
# transformer_decoder1（变换器解码器1）、transformer_decoder2（变换器解码器2）和fcn（全连接网络）

# 其中，位置编码器用于将输入的序列进行位置编码，以表示序列中元素的相对位置信息；变换器编码器用于将经过位置编码的输入序列转换为一个内部表示（memory）；
# 变换器解码器1和变换器解码器2分别用于对输入序列进行两次解码，得到不同的输出；全连接网络用于将解码器输出的内部表示转换为最终的输出。
...
class Transformer_Test(nn.Module):
    def __init__(self, feats):
        ...
        self.n_window = 10  # 滑动窗口
        self.n = self.n_feats * self.n_window
        self.pos_encoder = PositionalEncoding2(2 * feats, 0.1, self.n_window)
        encoder_layers = TransformerEncoderLayer2(d_model=2 * feats, nhead=feats, dim_feedforward=16, dropout=0.1)
        self.transformer_encoder = TransformerEncoder(encoder_layers, 1)
        decoder_layers1 = TransformerDecoderLayer2(d_model=2 * feats, nhead=feats, dim_feedforward=16, dropout=0.1)
        self.transformer_decoder1 = TransformerDecoder(decoder_layers1, 1)
        decoder_layers2 = TransformerDecoderLayer2(d_model=2 * feats, nhead=feats, dim_feedforward=16, dropout=0.1)
        self.transformer_decoder2 = TransformerDecoder(decoder_layers2, 1)
        self.fcn = nn.Sequential(nn.Linear(2 * feats, feats), nn.Sigmoid())
